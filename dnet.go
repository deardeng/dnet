...
// Conn is a interface of dnet connection.
type Conn interface {
	...
	// like the Content-Length attribute in an HTTP request which indicates you how much data you should read from
	// inbound ring-buffer.
...
type (
	...
	EventHandler interface {
		...
		// Invoke c.Read() or c.ReadN(n) within the parameter c to read incoming data from client/connections.
	...
	// EventServer is a built-in implementation of EventHandler which such sets up each method with a default implementation,
...
//
// The "tcp" network scheme is assumed when is not specified.
