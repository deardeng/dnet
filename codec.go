...
type (
	...
	// BuiltInFrameCodec is the build-in codec which will be assigned to dnet server when customized codec is not set up.
	...
	// LengthFieldBasedFrameCodec encodes/decodes frames into/from TCP stream with value of the length field in the message.
...
func NewLengthFieldBasedFrameCodec(encodeConfig EncoderConfig, decoderConfig DecoderConfig) *LengthFieldBasedFrameCodec {
	return &LengthFieldBasedFrameCodec{encoderConfig: encodeConfig, decoderConfig: decoderConfig}
}
...
func (cc *LengthFieldBasedFrameCodec) Encode(c Conn, buf []byte) (out []byte, err error) {
	...
	case 1:
		if length >= 256 {
			return nil, fmt.Errorf("length does not fit int a byte: %d", length)
		}
