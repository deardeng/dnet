...
type Options struct {
	...
	// if will run the server with single thread. The number of threads in the server will be automatically
	...
	// Note: Setting up NumEventLoop will override Nulticore.
	NumEventLoop int
...
// WithMulticore sets up multi-cores in dnet server.
...
// WithNumEventLoop sets up NumEventLoop in dnet server.
...
// WithTCPKeepAlive sets up SO_KEEPALIVE socket options.
